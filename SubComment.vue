<template>
  <div>
    ...
    <div class="children" v-for="(ritem,jndex) in childrenList">
      <div class="reply">
      </div>
      <div class="content">
        <div class="comment-f">
        ...
        <div class="comment-f">
          <div>
            ...
            <div v-if="parseInt(ritem.commentUser.id)===parseInt(authorId)" class="icon author">{{ label }}</div>
            <div class="date">
              {{ ritem.createDate }}
            </div>
        ...
        <div class="reply-content">
          <template v-if="ritem.targetUser">
            <div class="cc cc-to">
          ...
          <div class="comment" :style="{width:commentWidth}" v-if="replyMap[ritem.id]" :showAvatar="showAvatar">
            ...
            <div v-if="buttonMap[ritem.id]" class="hbl-owo">
              ...
              <div class="publish publish-btn">
                <button class="btn" @click="doChildSendUp(ritem.id,ritem.commentUser.id,jndex)">发送</button>
      ...
      <template v-if="ritem.childrenList">
        <sub-comment @doChildSendDown="doChildSendDown(arguments)" :childrenList="ritem.childrenList" :label="label" :avatar="avatar" :placeholder="placeholder" :minRows="minRows" :maxRows="maxRows" :author-id="authorId"></sub-comment>
      </template>
    </div>
  </div>
...
export default {
  ...
  methods: { //事件处理器
    ...
    doChildSendDown(args) {
      //this.$alert("转发 "+args[0]+" "+ args[1]+" "+ args[2]);
      this.$emit("doChildSendDown", args[0], args[1], args[2]);
    },

    doChildSendUp(index, commentUserId, pid) {
      console.log("index: "+index+"    pid: "+pid)
      //this.$alert("子模块"+this.textareaMap[index]+" "+ commentUserId+" "+ pid);
      this.$emit("doChildSendDown", this.textareaMap[index], commentUserId, index);
